...
while ($l = mysql_fetch_object($r)) {
    ...
    if (((int) $l->total) > 0)
    {
        ...
        $view_facs[$l->faculty_id]=$l->faculty_id;
        $view_tfms[$l->time_form_id]=$l->time_form_id;
    }
}
...
if($raw)
{
ui_sp('Дискретная таблица(Платная сводная)');
...
foreach ($fc_arr as $k => $v) {
    foreach ($tf_arr as $kk => $vv) {
        if ($view_paid[$k][$kk]) {
            ...
            $sqlnulls[] = "COUNT(IF(`faculty`='$k' AND `time_form_id`='$kk' AND `target` in (4,7),total,null)) as c$k$kk";
...
$forder=implode(', ', $view_facs);

$torder=implode(', ', $view_tfms);
...
$sql = <<<EOF
    ...
    (SELECT DISTINCT (ceil(total/10))*10 as t,
        $sqlc
    FROM $target_table WHERE $fid `faculty` in ($forder) AND `time_form_id` IN ($torder) AND `state_id`=1 AND `edu_form`=2 AND `target`=3 group by `t` order by `t` desc)
