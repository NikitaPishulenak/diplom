...
include 'form/person.column.group.php';
...
ui_stfs();
ui_sfs();
print '<p id="fields_v" style="width:150px"></p>';
ui_efs();
ui_sptfs3();
ui_stfs();
ui_sfs();
...
while($l=  mysql_fetch_object($r)){
    ...
    $g=(isset($pgroups[$l->Field]))?$pgroups[$l->Field]:'last-group';
    $mgroups[$g][]=$a;
}
ksort($mgroups);
...
{
    ui_rep_row();
    $gt=(isset($tgroups[$k]))?$tgroups[$k]:$k;
    ui_rep_th($gt);
    ui_end_row();
    foreach($v as $vv)
    {
        ui_rep_row();
        print "<td  style=\"white-space:nowrap;padding:1px;line-height:18px;\">$vv</td>";
        $i++;
        ui_end_row();
    }
    if($i>20)
