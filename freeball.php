...
//else
//    $wct="`target_use_id`=1";
$sql = "INSERT INTO `db_ball_temp` (`faculty`,`time_form_id`,`edu_form`,`target`,`target_type`,`target_cell`,`region_cell`,`total`,`delo_name`,`person_id`) SELECT `faculty`,`time_form_id`,`edu_form`,`target`,`target_type`,`target_cell`,`region_cell`,`total`,`delo_name`,$target_key from $target_table WHERE $fid $wct";
$r = mysql_query($sql) or debug($sql, mysql_error());
    ...
    foreach ($tc_arr as $kk => $vv) {
        ...
        foreach ($cr_arr as $kkk => $vvv) {
            ...
            if ($p == 0) {
                ...
                $i_c = 0;
                $sql = "SELECT `delo_name`,`target_type`,`id_ball_temp`,`total` FROM `db_ball_temp` WHERE `faculty`='$k' AND `time_form_id`=1 AND `edu_form`=1 AND `target`=1 AND `target_cell`='$kk' AND `region_cell`='$kkk' ORDER BY `total` DESC";
                $r = mysql_query($sql) or debug($sql, mysql_error());
                while ($l = mysql_fetch_object($r)) {
                    $i_c++;
...
while ($l = mysql_fetch_object($r)) {
    ...
    $k = $l->faculty;
    $sql = "SELECT `total` FROM `db_ball_temp` WHERE `faculty`='$k' AND `time_form_id`=1 AND `edu_form`=1 AND `target`=3 AND `target_type`=1 ORDER BY `total` DESC";
    $rr = mysql_query($sql) or debug($sql, mysql_error());
    $i_c = 0;
    $c_ball = 0;
    $cp_ball = '';
    ...
    /* СЕЛО */
    $sql = "SELECT `total` FROM `db_ball_temp` WHERE `faculty`='$k' AND `time_form_id`=1 AND `edu_form`=1 AND `target`=3 AND `target_type`=2 ORDER BY `total` DESC";
    $rr = mysql_query($sql) or debug($sql, mysql_error());
    $i_c = 0;
    $v_ball = 0;
    $vp_ball = '';
...
$sql = <<<EOF
    SELECT
        faculty,
        (SELECT sum(db_planform.total)  FROM db_planform  WHERE (db_planform.faculty_id=db_ball_temp.faculty AND `time_form_id`=2 AND `edu_form_id`=1)) as  planform,
        COUNT(IF(`target`=5,`total`,null)) as pmo,
        COUNT(IF(`target`=2,`total`,null)) as pbi,
        COUNT(IF(`target`=3,`total`,null)) as pgen,
        COUNT(IF(`target`=3 AND `target_type`=1,`total`,null)) as pgc,
       COUNT(IF(`target`=3 AND `target_type`=2,`total`,null)) as pgv
        FROM `db_ball_temp` WHERE  `time_form_id`=2 AND `edu_form`=1 group by faculty
EOF;
$r = mysql_query($sql) or debug($sql, mysql_error());
while ($l = mysql_fetch_object($r)) {
    ui_rep_row();
    ...
    ui_rep_td($l->pgc);
    ui_rep_td($l->pgv);
    $gen_mest = $l->planform - $l->pmo - $l->pbi;
    $gen_live = $l->planform - $l->pmo - $l->pbi;
    ui_rep_td($gen_live);
    if ($gen_live)
        $prop = $l->pgv * 100 / $l->pgen;
    else
        $prop = 0.5;
    $v_mest = round($gen_mest * $prop / 100);
    ...
    $k = $l->faculty;
    $sql = "SELECT `total` FROM `db_ball_temp` WHERE `faculty`='$k' AND `time_form_id`=2 AND `edu_form`=1 AND `target`=3 AND `target_type`=1 ORDER BY `total` DESC";
    $rr = mysql_query($sql) or debug($sql, mysql_error());
    $i_c = 0;
    $c_ball = 0;
    $cp_ball = '';
    ...
    /* СЕЛО */
    $sql = "SELECT `total` FROM `db_ball_temp` WHERE `faculty`='$k' AND `time_form_id`=2 AND `edu_form`=1 AND `target`=3 AND `target_type`=2 ORDER BY `total` DESC";
    $rr = mysql_query($sql) or debug($sql, mysql_error());
    $i_c = 0;
    $v_ball = 0;
    $vp_ball = '';
    ...
    /* FAIL */
    $sql = "SELECT `total` FROM `db_ball_temp` WHERE `faculty`='$k' AND `time_form_id`=2 AND `edu_form`=1 AND `target`=3 ORDER BY `total` DESC";
    $rr = mysql_query($sql) or debug($sql, mysql_error());
    $i_c = 0;
    $f_ball = 0;
    $fp_ball = '';
    ...
    /*
    ui_rep_td($c_ball . $cp_ball);
    ui_rep_td($v_ball . $vp_ball);
    ui_rep_th($f_ball . $fp_ball);
*/
