...
function list_draw()
{
    $sql="SELECT * FROM `db_target_use` ";
    $r=mysql_query($sql) or debug($sql,mysql_error());
    ui_ssfs();
    ui_sfs();
    ui_rowlink(0, 'Создать', "target_uses.php", array("id=0"));
    while($l=  mysql_fetch_object($r))
    {
        ui_rowlink($l->id_target_use, $l->name, "target_uses.php", array("id=$l->id_target_use"),array($l->sname,$l->abbr));
    }
    ui_efs();
    ui_esfs();
}