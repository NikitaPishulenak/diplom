...
function list_draw()
{
    $sql="SELECT * FROM `db_president` ";
    $r=mysql_query($sql) or debug($sql,mysql_error());
    ui_ssfs();
    ui_sfs();
    ui_rowlink(0, 'Создать', "presidents.php", array("id=0"));
    while($l=  mysql_fetch_object($r))
    {
        ui_rowlink($l->id_president, $l->surname, "presidents.php", array("id=$l->id_president"),array($l->name,$l->midname));
    }
    ui_efs();
    ui_esfs();
}