...
function list_draw()
{
    $sql="SELECT * FROM `db_db_klik` ";
    $r=mysql_query($sql) or debug($sql,mysql_error());
    ui_ssfs();
    ui_sfs();
    ui_rowlink(0, 'Создать', "db_kliks.php", array("id=0"));
    while($l=  mysql_fetch_object($r))
    {
        ui_rowlink($l->id_db_klik, $l->name, "db_kliks.php", array("id=$l->id_db_klik"),array($l->sname,$l->abbr));
    }
    ui_efs();
    ui_esfs();
}