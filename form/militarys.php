...
function list_draw()
{
    $sql="SELECT * FROM `db_military` ";
    $r=mysql_query($sql) or debug($sql,mysql_error());
    ui_ssfs();
    ui_sfs();
    ui_rowlink(0, 'Создать', "militarys.php", array("id=0"));
    while($l=  mysql_fetch_object($r))
    {
        ui_rowlink($l->id_military, $l->name, "militarys.php", array("id=$l->id_military"),array($l->sname,$l->abbr));
    }
    ui_efs();
    ui_esfs();
}