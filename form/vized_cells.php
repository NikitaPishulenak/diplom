...
function list_draw()
{
    $sql="SELECT * FROM `db_vized_cell` ";
    $r=mysql_query($sql) or debug($sql,mysql_error());
    ui_ssfs();
    ui_sfs();
    ui_rowlink(0, 'Создать', "vized_cells.php", array("id=0"));
    while($l=  mysql_fetch_object($r))
    {
        ui_rowlink($l->id_vized_cell, $l->name, "vized_cells.php", array("id=$l->id_vized_cell"),array($l->sname,$l->abbr));
    }
    ui_efs();
    ui_esfs();
}