...
function list_draw()
{
    $sql="SELECT * FROM `db_nametown` ";
    $r=mysql_query($sql) or debug($sql,mysql_error());
    ui_ssfs();
    ui_sfs();
    ui_rowlink(0, 'Создать', "nametowns.php", array("id=0"));
    while($l=  mysql_fetch_object($r))
    {
        ui_rowlink($l->id_nametown, $l->name, "nametowns.php", array("id=$l->id_nametown"),array($l->sname,$l->abbr));
    }
    ui_efs();
    ui_esfs();
}