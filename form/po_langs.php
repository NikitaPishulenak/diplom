...
function list_draw()
{
    $sql="SELECT * FROM `db_po_lang` ";
    $r=mysql_query($sql) or debug($sql,mysql_error());
    ui_ssfs();
    ui_sfs();
    ui_rowlink(0, 'Создать', "po_langs.php", array("id=0"));
    while($l=  mysql_fetch_object($r))
    {
        ui_rowlink($l->id_po_lang, $l->name, "po_langs.php", array("id=$l->id_po_lang"),array($l->sname,$l->abbr));
    }
    ui_efs();
    ui_esfs();
}