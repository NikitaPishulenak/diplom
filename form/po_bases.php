...
function list_draw()
{
    $sql="SELECT * FROM `db_po_base` ";
    $r=mysql_query($sql) or debug($sql,mysql_error());
    ui_ssfs();
    ui_sfs();
    ui_rowlink(0, 'Создать', "po_bases.php", array("id=0"));
    while($l=  mysql_fetch_object($r))
    {
        ui_rowlink($l->id_po_base, $l->name, "po_bases.php", array("id=$l->id_po_base"),array($l->sname,$l->abbr));
    }
    ui_efs();
    ui_esfs();
}