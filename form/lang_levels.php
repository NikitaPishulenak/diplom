...
function list_draw()
{
    $sql="SELECT * FROM `db_lang_level` ";
    $r=mysql_query($sql) or debug($sql,mysql_error());
    ui_ssfs();
    ui_sfs();
    ui_rowlink(0, 'Создать', "lang_levels.php", array("id=0"));
    while($l=  mysql_fetch_object($r))
    {
        ui_rowlink($l->id_lang_level, $l->name, "lang_levels.php", array("id=$l->id_lang_level"),array($l->sname,$l->abbr));
    }
    ui_efs();
    ui_esfs();
}