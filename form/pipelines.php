...
function list_draw()
{
    $sql="SELECT * FROM `db_pipeline` ";
    $r=mysql_query($sql) or debug($sql,mysql_error());
    ui_ssfs();
    ui_sfs();
    ui_rowlink(0, 'Создать', "pipelines.php", array("id=0"));
    while($l=  mysql_fetch_object($r))
    {
        ui_rowlink($l->id_pipeline, $l->name, "pipelines.php", array("id=$l->id_pipeline"),array($l->sname,$l->abbr));
    }
    ui_efs();
    ui_esfs();
}