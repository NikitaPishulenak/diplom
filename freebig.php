...
/** @link ++ ROUND 4 ++ */
ui_sfs('Проходной балл целевого приёма', 'w100','ballcell');
ui_rep_row();
ui_rep_th('Факультет');
ui_rep_th('ТЦ');
foreach ($cr_arr as $kkk => $vvv) {
...
/** @link ++ ROUND 7 ++ */
ui_sfs('Общий конкурс', 'w100','ballgens');
ui_rep_row();
ui_rep_th('Факультет');
...
$sql = <<<EOF
    SELECT
        ...
        FROM `db_ball_temp` where `faculty` < 10 group by faculty
EOF;
$r = mysql_query($sql) or debug($sql, mysql_error());
while ($l = mysql_fetch_object($r)) {
    ui_rep_row();
...
//    $gen_mest = $l->planform - $cell_mest[$l->faculty] - $mo___mest[$l->faculty] - $bi___mest[$l->faculty];
    $gen_mest = $l->planform - $l->plancell - $mo___mest[$l->faculty] - $bi___mest[$l->faculty];
...
ui_script_start();
print<<<EOF
$('head').append('<link rel="stylesheet" type="text/css" href="css/big.css">');
EOF;
ui_script_end();
