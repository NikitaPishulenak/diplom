...
if(!empty($_POST))
{
    if($id>0)
    {
        ...
        $sql="INSERT INTO `db_fax` (${res[0]}) VALUES (${res[1]})";
        $r=mysql_query($sql) or debug($sql,  mysql_error());
        ui_redirect("fax.php?id=$id");
    }
}
...
//    ui_text('reg', 'call_request', 'Причина звонка', '', '');
    ...
    ui_ef();
    ui_hr();
    $sql="SELECT * FROM `db_fax` WHERE `person_id`='$id'";
    $r=mysql_query($sql) or debug($sql,  mysql_error());
    
    ui_sfs();
        ui_rep_row('','w100');
        ui_rep_th('#');
        ui_rep_th("Дата факс",'','','nw');
        ui_rep_th('Работник');
