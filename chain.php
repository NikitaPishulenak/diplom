...
if ($id > 0) {
    ...
    ui_sp('Цепочка: ' . $p['name']);
    $sql = "SELECT * FROM `db_person` WHERE (`id` in (SELECT `person_id` FROM `db_chid` WHERE(`chain_id`='$id')))";
    $r = mysql_query($sql) or debug($sql, mysql_error());
...
EOF;
    ...
    if (isset($_GET['new'])) {
        ...
        ui_sp('Цепочки');
        $uid = cr_userid();
        $sql = "SELECT *,(SELECT COUNT(`id_chid`) FROM `db_chid` WHERE `chain_id`=id_chain) ch_cnt FROM `db_chain` WHERE `owner_id`='$uid'";
        $r = mysql_query($sql) or debug($sql, mysql_error());
        ui_sf('cop.php');
        ...
        $i = 0;
        while ($l = mysql_fetch_object($r)) {
            $el = "<a href=\"chain.php?id=$l->id_chain\">$l->name</a>";
            ui_rep_row();
            ui_rep_th(++$i);
            ui_rep_th('<input type="checkbox" name="reg[' . $l->id_chain . ']"/>');
