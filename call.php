...
if(!empty($_POST))
{
    if($id>0)
    {
        ...
        $sql="INSERT INTO `db_call` (${res[0]}) VALUES (${res[1]})";
        $r=mysql_query($sql) or debug($sql,  mysql_error());
        ui_redirect("call.php?id=$id");
    }
}
...
if ($id > 0) {
    ...
    ui_text_view('reg', 'f_phone','Телефон', $p['f_phone'],50);
    ui_efs();
    ui_sptfs3();
    ui_sfs('Мать');
    ...
    ui_text_view('reg', 'm_phone','Телефон', $p['m_phone'],50);
    ...
    ui_text('reg', 'call_response', 'Результат звонка', '', '');
    ui_hidden('reg','name',$p['delo_name']);
    ui_efs();
    ui_ef();
    ui_hr();
    $sql="SELECT * FROM `db_call` WHERE `person_id`='$id'";
    $r=mysql_query($sql) or debug($sql,  mysql_error());
    
    ui_sfs();
        ui_rep_row('','w100');
        ui_rep_th('#');
        ui_rep_th("Дата звонка",'','','nw');
        ui_rep_th('Работник');
        ui_rep_th('Причина','','','w50');
        ui_rep_th('Результат','','','w50');
    ...
    while($l=  mysql_fetch_object($r))
    {
        ...
        ui_rep_td_l($l->call_response,'w50');
