...
define('PAGE_SEC', 'control.formacl');
cr_logic();
...
if(!empty($_POST))
{
    ...
    else
    {
        ...
        $sql="INSERT INTO `db_target_formable` (${res[0]}) VALUES (${res[1]})";
        $r=mysql_query($sql) or debug($sql,  mysql_error());
    }
    ui_redirect("formacl.php");
}
...
if($id)
    $p=db_single_record('db_target_formable', 'id_target_allowable',$id );
else
    $p=db_default_record ('db_target_formable');

ui_sp('Допустимый конкурс форм');
...
while($l=mysql_fetch_object($r))
{
    ...
    ui_rep_td('<a href="api.php?m=facl.del&amp;i='.$l->id_target_formable.'">x</a>');
    ui_end_row();
}
ui_efs();
